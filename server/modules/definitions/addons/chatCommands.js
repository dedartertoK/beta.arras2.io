...
class Commands {
	...
	checkPerms(command, socket) {
		...
		if (command.perms > 0 && !permissions) {
			command.status('fail').send('Unknown command.')
			return false;
		}
...
module.exports = ({ Config, Events }) => {
	...
	commands.add('nobot', 0, {doc: 'Kill all the bots'}, ({command}) => {
		sockets.broadcast("Bots disabled.");
		c.BOTS = false;
		    // Remove bots
	...
	commands.add(['god', 'invuln'], 0, { doc: 'Toggle invulnerable mode\nExample: /god' }, ({ body }) =>
	...
	commands.add({
		...
		execute: ({ args, body, command }) => {
			...
			// Check if color value is higher than 41
			if (typeof color === 'string' && parseInt(color) > 41) {
				return command.send('Invalid color. Color value must be 41 or lower.');
			}
