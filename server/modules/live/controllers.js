...
class io_bossRushAI extends IO {
    ...
    think(input) {
        ...
        if (this.enabled) {
            if (this.goal.type !== "bas1") {
                this.goal = room.randomType("bas1", this.body);
            }
            return {
                main: false,
                alt: false,
...
class io_listenToPlayer extends IO {
    ...
    think() {
        ...
        if (this.body.aiSettings.isDigger) {
            ...
            let av = Math.sqrt(target.x * target.x + target.y * target.y);
            let x = target.x / (av - 1);
            let y = target.y / (av - 1);
...
class io_nearestDifferentMaster extends IO {
    constructor(body, opts = {}) {
        ...
        if (body && body.health && body.health.display) {
            this.oldHealth = body.health.display();
        } else {
            // Handle the case where body, body.health, or body.health.display is undefined
            console.error("Cannot read health properties.");
        }
...
class io_spinWhileIdlemiaow extends IO {
    constructor(b) {
      super(b);
      this.a = 0;
    }
...
let ioTypes = {
    ...
    multibox: io_multibox,
    multiboxClone: io_multiboxClone,
    ...
    spinWhileIdlemiaow: io_spinWhileIdlemiaow,
    spin: io_spin,
    reverseSpin: io_reverseSpin,
    slowSpin: io_slowSpin,
    slowSpinReverse: io_slowSpinReverse,
    orbitAroundPlayer: io_orbitAroundPlayer,
