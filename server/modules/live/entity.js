...
class Entity extends EventEmitter {
    ...
    define(defs, emitEvent = true) {
        ...
        if (set.GODMODE != null) this.godmode = set.GODMODE;
    ...
    bindToMaster(position, bond, isInvulnerable) {
        ...
        if (isInvulnerable) this.onDeath = () =>  {util.remove(this.master.turrets, this.master.turrets.indexOf(this))}
    ...
    confinementToTheseEarthlyShackles() {
        ...
        if (c.SPECIAL_BOSS_SPAWNS && (this.type === "tank" || this.type === "food") && room.isIn("outb", this) && this.master.label !== "Arena Closer") {
            this.kill();
        }
        ...
        if (this.isDead()) {
            ...
            if (this.settings.defeatMessage) {
                ...
                if (c.MOTHERSHIP_LOOP) {
                    text += "Mothership fought a polygon... and the polygon won."
                }
            ...
            if (arenaClosed) {
                killText = "You have died by the Arena Closers";
            } else {
    ...
    toggleRainbow() {
        if (!this.rainbow) {
            // Rainbow mode is turned off, so disable any rainbow-related functionality
            clearInterval(this.intervalID);
            // Additional logic to reset color or perform any other cleanup if needed
            this.color = 10;
        } else {
            // Rainbow mode is turned on
    ...
    rainbowLoop() {
        if (this.color < 36 || isNaN(this.color)) this.color = 36;
