...
const gameloop = () => {
    ...
    if (ticks & 1) {
        for (let i = 0; i < sockets.players.length; i++) {
            if (sockets.players[i] && sockets.players[i].socket && sockets.players[i].socket.view) {
...
const doors = [];
const buttons = [];
// Inside the Entity class or relevant properties
Entity.prototype.entitiesInside = [];

// Inside the makeDoor function
function makeDoor(loc, team = -100) {
    const door = new Entity(loc);
    door.define(Class.wall1);
    door.define({ DANGER: -7 });
    door.team = -100;
    ...
    door.entitiesInside = []; // Initialize entities inside the door

    door.enterDoor = function (entity) {
        // Add the entity to the list of entities inside the door
        door.entitiesInside.push(entity);

        // Implement logic for damaging entities inside the door (adjust as needed)
        entity.damage(10); // Adjust the amount of damage as needed
    };
...
function makeButton(loc, open, doorID) {
    ...
    button.color = open ? 11 : 12;
    
    button.onDeath = function () {




// Inside the button.onDeath function
if (!button.ignoreButtonKill) {
    ...
    if (open) {
        ...
        // Call enterDoor for each entity inside the door
        for (const entityInside of door.entitiesInside) {
            door.enterDoor(entityInside);
        }

        // Kill the corresponding wall on button death
...
for (const loc of room.door) {
    makeDoor(loc);
    let buttonLocs = [{
        x: loc.x + (room.width / room.xgrid) / 1.7,
        y: loc.y
    }, {
        x: loc.x - (room.width / room.xgrid) / 1.7,
        y: loc.y
        ...
        x: loc.x,
        y: loc.y + (room.height / room.ygrid) / 1.7,
    }, {
        x: loc.x,
        y: loc.y - (room.height / room.ygrid) / 1.7,
    }];
...
if (c.DDAY_LOOP || c.DDAYTWO_LOOP) {
    function spawnWall1(loc) {
        let o = new Entity(loc);
        o.define(Class.wall);
        o.team = TEAM_ROOM;
        o.SIZE = room.width / room.xgrid / 2;
        ...
        // Set a timeout to destroy the entity after 1 minute (60000 milliseconds)
        setTimeout(() => {
            // Assuming you have a method named 'destroy' to handle entity destruction
            o.destroy();
        }, 60000);
    }
    for (let loc of room["wal1"]) spawnWall1(loc);
    }
...
{
    bosses: [Class.fallenBoosterAI],
    location: "norm",
    amount: [1],
    nameType: "a",
    message: "A Fallen Booster has spawned!",
    chance: 1,
},
{
...
spawnBots = () => {
    ...
    if (!global.arenaClosed && bots.length < c.BOTS) {
        ...
        if (c.SIEGE_LOOP) {
            o.DANGER = 0; // Set DANGER to 0 for bots when c.SIEGE_LOOP is true
        }
        bots.push(o);
