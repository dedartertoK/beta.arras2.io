...
let animations = window.animations = {
    ...
    disconnected: new Animation(0, 0),
    deathScreen: new Animation(0.5, 0),
...
global.tips = [
"Tip: For better performance and have a smoother experience, enable better FPS on the view options!",
"Tip: You can press T to see the class tree",
"Want to access to the closed beta server? Contact the developer!",
"Any cheating scripts such as multibox will cause in a ban.",
...
"If you showcase the game, you can have access to the closed beta server!",
...
var betterPerformanceCheckbox = document.getElementById("betterperformance");
...
betterPerformanceCheckbox.addEventListener("change", function() {
    ...
    localStorage.setItem("betterperformance", betterPerformanceCheckbox.checked);
});
...
if (betterPerformanceChecked !== null) {
    ...
    document.onkeydown = (e) => {
        ...
        if (key === global.KEY_ENTER && !global.connecting && (global.dead || !global.gameStart)) {
            startGame();
        }
...
function startGame() {
    ...
    config.graphical.highResolution = document.getElementById("high").checked;
...
const drawEntity = (baseColor, x, y, instance, ratio, alpha = 1, scale = 1, rot = 0, turretsObeyRot = false, assignedContext = false, turretInfo = false, render = instance.render) => {
    ...
    var autoLevelUpCheckbox = document.getElementById("autoLevelUp");
    ...
    function checkAutoLevelUp() {
        ...
        // Update the autoLevelUp variable
        config.game.autoLevelUp = true;
    ...
    // Call the function to check the checkbox automatically
    checkAutoLevelUp();
    function drawHealth(x, y, instance, ratio, alpha) {
        ...
        if (instance.id !== gui.playerid && instance.nameplate) {
            ...
            // Adjust font size based on entity size
            ...
            drawText(name, x, y - realSize - 20 * nameRatio, 16 * nameSize, namecolor, "center"); 
            drawText(util.handleLargeNumber(instance.score, 1), x, y - realSize - 8 * nameRatio, 16 * scoreSize, namecolor, "center");
...
function drawFloor(px, py, ratio) {
    ...
    for (let i = 0; i < H; i++) {
        ...
        for (let j = 0; j < W; j++) {
            ...
            //draw it
            let tile = row[j];
            let left = (ratio * j * global.gameWidth) / W - px + global.screenWidth / 2,
                right = (ratio * (j + 1) * global.gameWidth) / W - px + global.screenWidth / 2,
                top = (ratio * i * global.gameHeight) / H - py + global.screenHeight / 2,
                bottom = (ratio * (i + 1) * global.gameHeight) / H - py + global.screenHeight / 2;
...
function drawEntities(px, py, ratio) {
    ...
    for (let instance of global.entities) {
        if (!(instance.id === gui.playerid) && !global.showInvisible && instance.alpha < 0.25) continue;
...
function drawMessages(spacing) {
    ...
    // Calculate total height occupied by global.messages1
    let messages1Height = global.messages1.length * (vspacing + height);
    let messages2Height = global.messages2.length * (vspacing + height);
    let messages3Height = global.messages3.length * (vspacing + height);
    let messages4Height = global.messages4.length * (vspacing + height);
    let messages5Height = global.messages5.length * (vspacing + height);
    let y = spacing + messages1Height + messages2Height + messages3Height + messages4Height + messages5Height; // Start y position below global.messages1
    ...
    for (let i = global.messages.length - 1; i >= 0; i--) {
        ...
        if (msg.status > 1) {
            ...
            msg.status -= 0.05 * deltaTime /  REFERENCE_DELTA_TIME; // Decrease faster
            msg.alpha += 0.05 * deltaTime / REFERENCE_DELTA_TIME; // Increase faster
        } else if (
            i === 0 &&
            (global.messages.length > 5 || Date.now() - msg.time > 10000)
        ) {
            msg.status -= 0.05 * deltaTime / REFERENCE_DELTA_TIME; // Decrease faster
            msg.alpha -= 0.05 * deltaTime / REFERENCE_DELTA_TIME; // Decrease faster
...
function drawMessages1(spacing) {
    ...
    // Calculate total height occupied by global.messages2
    let messages2Height = global.messages2.length * (vspacing + height);
    let messages3Height = global.messages3.length * (vspacing + height);
    let messages4Height = global.messages4.length * (vspacing + height);
    let messages5Height = global.messages5.length * (vspacing + height);
    let y = spacing + messages2Height + messages3Height + messages4Height + messages5Height; // Start y position below global.messages2
    // Draw each message
    for (let i = global.messages1.length - 1; i >= 0; i--) {
        let msg = global.messages1[i];
        let txt = msg.text;
        let text = txt;
        ...
        // Draw the background
        ctx.globalAlpha = 1; // Set alpha to 1 (no alpha transition)
        drawBar(x - msg.len / 2, x + msg.len / 2, y + height / 2, height, "#FF0000");
...
function drawMessages2(spacing) {
    ...
    let messages4Height = global.messages4.length * (vspacing + height);
    let messages5Height = global.messages5.length * (vspacing + height);
    let y = spacing  + messages3Height + messages4Height + messages5Height; // Start y position below global.messages2



    // Draw each message
    for (let i = global.messages2.length - 1; i >= 0; i--) {
        let msg = global.messages2[i];
        let txt = msg.text;
        let text = txt;
        ...
        drawBar(x - msg.len / 2, x + msg.len / 2, y + height / 2, height, "#00B0E1");
...
function drawMessages3(spacing) {
    ...
    let messages4Height = global.messages4.length * (vspacing + height);
    let messages5Height = global.messages5.length * (vspacing + height);
    let y = spacing  +  messages4Height + messages5Height; // Start y position below global.messages2



    // Draw each message
    for (let i = global.messages3.length - 1; i >= 0; i--) {
        let msg = global.messages3[i];
        let txt = msg.text;
        let text = txt;
        ...
        drawBar(x - msg.len / 2, x + msg.len / 2, y + height / 2, height, "#FF5E65");
...
function drawMessages4(spacing) {
    ...
    let x = global.screenWidth / 2;
    let messages5Height = global.messages5.length * (vspacing + height);
    let y = spacing   + messages5Height; // Start y position below global.messages2


    // Draw each message
    for (let i = global.messages4.length - 1; i >= 0; i--) {
        let msg = global.messages4[i];
        let txt = msg.text;
        let text = txt;
        ...
        drawBar(x - msg.len / 2, x + msg.len / 2, y + height / 2, height, "#FFF83B");
...
function drawMessages5(spacing) {
    ...
    for (let i = global.messages5.length - 1; i >= 0; i--) {
        ...
        drawBar(x - msg.len / 2, x + msg.len / 2, y + height / 2, height, "#5C6BC0");
...
function drawMinimapAndDebug(spacing, alcoveSize) {
    ...
    function interpolateColor(color1, color2, factor) {
        const result = color1.map((value, index) => {
            const blendedValue = Math.round(value + factor * (color2[index] - value));
    ...
    if (global.showDebug) {
        ...
        drawText("Arras2.io", x + len, y - 50 - 5 * 14 - 2, 15, rainbowColor, "right");
        ...
        drawText(global.metrics.latency + " ms - " + global.serverName, x + len, y - 50, 10, color.guiwhite, "right");
         ...
         drawText("Arras2.io", x + len, y - 50 - 2 * 14 - 2, 15, rainbowColor, "right");
         drawText((100 * gui.fps).toFixed(2) + "% : " + global.metrics.rendertime + " FPS", x + len, y - 50 - 1 * 14, 10, global.metrics.rendertime > 10 ? color.guiwhite : color.orange, "right");
...
let getDeath = () => {
  ...
  function getKillerPosition() {
    let lastAttacker = { x: 0, y: 0 };
    if (lastAttacker && lastAttacker.x && lastAttacker.y) {
      return { killerX: lastAttacker.x, killerY: lastAttacker.y };
    } else {
      // Handle case where lastAttacker information is not found (optional)
      return { killerX: 0, killerY: 0 }; // Or throw an error
    }
  }
const gameDrawDead = (killerName, killerPosition = { x: 0, y: 0 }) => {
  clearScreen(color.black, 0.75);
  ...
  // Get player and killer positions
  let playerX = global.player.x; 
  let playerY = global.player.y;
  let { killerX, killerY } = getKillerPosition();

  // Calculate camera shift based on difference between positions
  let shift = Math.min(0.5, Math.abs(global.player.y - killerPosition.y) / global.screenHeight);

    // Draw tank icon and score
    let x = global.screenWidth / 2,
        y = global.screenHeight / 1.8;

    let len = 140,
        position = global.mockups[parseInt(gui.type.split("-")[0])].position,
        scale = len / position.axis,
        xx = global.screenWidth / 1.8 - scale * position.middle.x * 0.707,
        yy = global.screenHeight / 2 - 215 + scale * position.middle.x * 0.707,
        picture = util.getEntityImageFromMockup(gui.type, gui.color),
        baseColor = picture.color;

    // Calculate the length of the tank name text
    let tankName = "Level " + gui.__s.getLevel() + " " + picture.name;
    ...
    drawEntity(baseColor, (xx - 300 - len / 2 + 0.5) | 0, (yy - 10 + 0.5) | 0, picture, 1.5 * tankScale, 1 * tankScale, (0.5 * scale) / picture.realSize, -Math.PI / 4, true);

    // Draw other messages
    drawText("Oh No, You died!", x, y - 150, 40, color.red, "center");
    drawText("You have been killed by " + getDeath() + ".", x, y - 100, 24, color.guiwhite, "center");
    drawText("But don't give up!", x, y - 70, 24, color.guiwhite, "center");
    drawText("You can do better next time!", x, y - 30, 24, color.guiwhite, "center");
    drawText(tankName, x, y - 260, 24, color.guiwhite, "center"); // Tank name text with fixed size
    drawText("Your Name: " + (global.player.name || "Unnamed Player"), x, y - 290, 24, color.guiwhite, "center");
    drawText("Your score: " + util.formatLargeNumber(Math.round(global.finalScore.get())), x, y + 20, 24, color.guiwhite, "center");
    drawText("⌚ Survived for " + util.timeForHumans(Math.round(global.finalLifetime.get())), x, y + 60, 16, color.guiwhite, "center");

    // Adjusted y-coordinate for "Press enter to rise again" text
    drawText("(press enter to rise again)", x, y + 100, 20, color.guiwhite, "center");

    ctx.translate(0, -shift * global.screenHeight);

    // Rest of the code for drawing the death screen remains the same...
...
const gameDrawErrorScreen = (e) => {
    ...
    ctx.translate(0, -shift * global.screenHeight);
    drawText(
      "OOPS, An error has been detected!",
    ...
    drawText(
      "This screen will disappear until the GUI error is gone. If it doesn't disappear, try to move away of the glitched entity!",
      global.screenWidth / 2,
    ...
    drawText(
        "SEND THIS REFERENCE ERROR TO THE DEVELOPER IN ORDER TO FIX IT!",
        global.screenWidth / 2,
        global.screenHeight / 2 + 120,
        15,
        "#FF0000",
        "center"
      );
...
const gameDrawDisconnected = () => {
    ...
    const dateTime = getCurrentDateTimeID();

    drawText(dateTime.dateID + ' ' + dateTime.time + ' ' + dateTime.timeZone, global.screenWidth / 2, global.screenHeight / 3 - 8, 8, color.guiwhite, "center");
    
    drawText("Disconnected", global.screenWidth / 2, global.screenHeight / 2.025, 31, color.guiwhite, "center");


    drawText(global.message || "Failed to validate your connection. This is most likely because the server has an issue or your connection is bad. Please try another server or clear your cache.", global.screenWidth / 2, global.screenHeight / 2 + 30, 15, color.orange, "center", 1); // Lower stroke width value, here set to 1


        if (global.allthesanctuarieskilled) drawText(global.sancdiedripforyouanyways || "", global.screenWidth / 2, global.screenHeight / 2 + 30, 15, color.orange, "center");
    ctx.translate(0, shift * global.screenHeight);
};
...
function animloop() {
    ...
    if (global.gameStart && !global.disconnected) {
        global.time = getNow();
        if (document.getElementById("betterperformance").checked) {
